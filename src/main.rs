...
impl<V> Ast<V> {
    ...
    fn label(label: impl ToString, value: Self) -> Self {
        Self::Label(label.to_string(), Box::new(value))
    }

    fn unlabel(value: Self, label: impl ToString) -> Self {
        Self::Unlabel(Box::new(value), label.to_string())
    }

    fn project(dir: Direction, value: Self) -> Self {
    ...
    fn inject(dir: Direction, value: Self) -> Self {
...
impl ClosedRow {
    fn merge(left: ClosedRow, right: ClosedRow) -> ClosedRow {
    ...
    fn mentions(&self, unbound_tys: &HashSet<TypeVar>, unbound_rows: &HashSet<RowVar>) -> bool {
...
impl Row {
    fn single(lbl: Label, ty: Type) -> Self {
    ...
    fn equatable(&self, other: &Self) -> bool {
        match (self, other) {
...
impl Type {
    ...
    fn row(label: Label, value: Self) -> Self {
        Self::Row(label, Box::new(value))
    }
    ...
    fn occurs_check(&self, var: TypeVar) -> Result<(), Type> {
        match self {
            ...
            Type::Row(_, ty) => ty.occurs_check(var).map_err(|_| self.clone()),
    ...
    fn mentions(&self, unbound_tys: &HashSet<TypeVar>, unbound_rows: &HashSet<RowVar>) -> bool {
        match self {
            ...
            Type::Row(_, ty) => ty.mentions(unbound_tys, unbound_rows),
            Type::Prod(row) | Type::Sum(row) => match row {
...
impl RowEquation {
    ...
    fn is_comm_unifiable(&self, other: &Self) -> bool {
...
impl TypeInference {
        ...
        match ast {
            ...
            Ast::Label(label, value) => {
                ...
                (
                    ...
                    Type::row(label, value_ty),
                )
            }
            Ast::Unlabel(value, label) => {
                let value_var = self.fresh_ty_var();
                let expected_ty = Type::row(label, Type::Var(value_var));
    ...
    fn check(&mut self, env: im::HashMap<Var, Type>, ast: Ast<Var>, ty: Type) -> GenOut {
        match (ast, ty) {
            ...
            (ast @ Ast::Concat(_, _), Type::Row(lbl, ty))
            | (ast @ Ast::Project(_, _), Type::Row(lbl, ty)) => {
                // Cast a singleton row into a product
                self.check(env, ast, Type::Prod(Row::single(lbl, *ty)))
            }
            (ast @ Ast::Branch(_, _), Type::Row(lbl, ty))
            | (ast @ Ast::Inject(_, _), Type::Row(lbl, ty)) => {
...
impl TypeInference {
    ...
    fn unify_ty_ty(&mut self, unnorm_left: Type, unnorm_right: Type) -> Result<(), TypeError> {
        ...
        match (left, right) {
            ...
            (Type::Row(field, ty), Type::Prod(row))
            | (Type::Prod(row), Type::Row(field, ty))
            | (Type::Row(field, ty), Type::Sum(row))
            | (Type::Sum(row), Type::Row(field, ty)) => self.unify_row_row(
                Row::Closed(ClosedRow {
                    fields: vec![field],
                    values: vec![*ty],
                }),
...
impl TypeInference {
    ...
    fn substitute_ty(&mut self, ty: Type) -> SubstOut<Type> {
        match ty {
            ...
            Type::Row(field, value) => self.substitute_ty(*value).map(|ty| Type::row(field, ty)),
            Type::Prod(row) => self.substitute_row(row).map(Type::Prod),
            Type::Sum(row) => self.substitute_row(row).map(Type::Sum),
        }
    }

    fn substitute_ast(&mut self, ast: Ast<TypedVar>) -> SubstOut<Ast<TypedVar>> {
        match ast {
            Ast::Var(v) => self
                .substitute_ty(v.1)
    ...
    fn substitute_row_eqn(&mut self, eqn: RowEquation) -> SubstOut<Evidence> {
